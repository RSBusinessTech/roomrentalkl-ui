...
export class PropertyTypeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit():void {
   ...
   this.checkReveal();
   this.rentalPropertiesService.getRentalProperties(this.propertySlug)
   .subscribe({
      next:(response)=>{
        console.log(
          'Rental API:',
        ...
        this.totalProperties = this.latestRooms.length;
        this.loading=false;
      },
  ...
  loadPropertyPage():void {
    this.propertySEO = PROPERTY_SEO[this.propertySlug];
    if(!this.propertySEO){
      console.error(
